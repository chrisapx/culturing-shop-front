...
const setCurrentOrderId = (accessToken: string) => {
  localStorage.setItem("current_order_id", accessToken);
};
...
const getCurrentOrderId = () => {
  return localStorage.getItem("current_order_id") ?? null;
};
...
const isAuthTokenExpired = (expirationTime: any) => {
